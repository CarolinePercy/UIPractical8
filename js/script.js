...
var facingRight = true; // keeps track of which direction the motorcycle is going
...
var yFrame = 0; // holds animation value along y axis for Npc (players y axis value stays constant)
  ...
  context.clearRect(0, 0, canvas.width, canvas.height); // clear canvas of previous frame
    ...
    if (current - initial >= 100) // check is greater that 500 ms
    {
      initial = current; // reset initial
        ...
        {
          yFrame += gameobjects[1].img.height / fireFrames;
        }

        if (xFrame[1] == 4 && yFrame == 350) // resets the frames once they reach the end of the sprite sheet
        {
          xFrame[1] = 0;
          yFrame = 0;
        }
    ...
    if (!facingRight) // if the player is facing left, show sprite to match
    {
      context.translate(gameobjects[0].x+gameobjects[0].img.width - 200, gameobjects[0].y);
      context.scale(-1,1);

      context.drawImage(gameobjects[0].img, (gameobjects[0].img.width / 3) * xFrame[0], 100, gameobjects[0].width,
    ...
    context.drawImage(gameobjects[1].img, (gameobjects[1].img.width / 8) * xFrame[1], yFrame.valueOf(), // draw NPC
    gameobjects[1].width, gameobjects[1].height, gameobjects[1].x, gameobjects[1].y, 50, 50);
}
